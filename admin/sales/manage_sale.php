    ...
    <div class="container-fluid">
        <div class="card card-outline card-outline rounded-0 shadow blur">
                            ...
                            <div class="d-flex h-100 w-100">
                                    ...
                                    <div class="card card-primary bg-transparent border-0 h-100 card-tabs rounded-0">
                                        <div class="card-header bg-gradient-dark p-0 pt-1">
                                            <ul class="nav nav-tabs" id="custom-tabs-one-tab" role="tablist">
                                                ...
                                                $product = $conn->query("SELECT * FROM `product_list` where delete_flag = 0 and `status` = 1 AND qty > 0 order by `name` asc");
                                                $prod_arr = [];
                                                        ...
                                                        <?php if(isset($prod_arr[$k])): ?>
                                                            <?php foreach($prod_arr[$k] as $row): ?>
                                                                <div class=" col-lg-3 col-md-4 col-sm-12 col-xs-12 px-2 py-3">
                                                                    <a href="javascript:void(0)"  class="card rounded-pill text-dark text-decoration-none prod-item" data-price="<?= $row['price'] ?>" data-id="<?= $row['id'] ?>" data-qty="<?= $row['qty'] ?>">
                                ...
                                <div class="col-5 h-100">
                                    ...
                                    <h6 class="text-light w-100 d-flex">
                                        ...
                                        <div class="col-auto flex-shrink-1 flex-grow-1 truncate-1 text-right" id="vat"><?= isset($amount) ? format_num($amount) : '0.00' ?></div>
                                    </h6>
                                    <h5 class="d-flex w-100 align-items-center">
                                        <div class="col-4" style="font-size: 20px">Tendered:</div>
                                        <div class="col-8">
                                            <input type="text" pattern="[0-9\.]*$" style="height: 25px" class="form-control form-control-lg rounded-0 text-right" id="tendered" name="tendered" value="<?= isset($tendered) ? str_replace(",", "", format_num($tendered)) : '0' ?>" required />
                                        </div>
                                    </h5>
                                    <h5 class="d-flex w-100 align-items-center">
                                        <div class="col-4" style="font-size: 20px">Change:</div>
                                        <div class="col-8">
                                            <input type="text" pattern="[0-9\.]*$" style="height: 25px" class="form-control form-control-lg rounded-0 text-right" id="change" value="<?= isset($amount) && isset($tendered) ? format_num($tendered - $amount) : '0' ?>" readonly />
                                        </div>
                                    </h5>
                                    <h5 class="d-flex w-100 align-items-center">
                                        <div class="col-4" style="font-size: 20px">Type:</div>
                                        <div class="col-8">
            ...
            <div class="card-footer py-2 text-right">
                <button class="btn btn-primary rounded-50" form="sale-form">Save sale</button>
                <?php if(!isset($id)): ?>
                <a class="btn btn-primary rounded-50" href="./?page=sales">Cancel</a>
                <?php else: ?>
                <a class="btn btn-primary rounded-50" href="./?page=sales/view_details&id=<?= $id ?>">Cancel</a>
                <?php endif; ?> 
            </div>
        </div>
    </div>
...
<script>
    ...
    $(function(){
        ...
        $('.prod-item').click(function(){
			...
			tr.find('input[name="product_qty[]"]').prop('max',qty);
        ...
        $('#sale-form').submit(function(e){
			...
			var cChange =$("#change").val();
			let output = parseFloat(cChange.replace(/,/g, ''));
			
			if (output < 0){
				alert("Tendered amount cannot be less than total due.");
				return;
			}
			
			var dataString = "client_name=" + clientname + "";
			...
			$.ajax({
				type: "POST",
				url: _base_url_+"classes/checkbalance.php",
				data: dataString,
				crossDomain: true,
				cache: false,
				success: function(data) {
					...
					if (withBalance !== true){
						//alert("Walang Balance!");						
					}
					else{
						alert(clientname + " still has balance, cannot proceed!");
					}
				},
