<?php
$date = isset($_GET['date']) ? $_GET['date'] : date("m");
...
<div class="card card-outline rounded-0 card-navy">
	<div class="card-header">
		<h3 class="card-title">Weekly Sales Report</h3>
	</div>
	<div class="card-body">
            ...
            <form id="filter-form" action="">
                <div class="row align-items-end">
                    <div class="col-lg-4 col-md-6 col-sm-12 col-xs-12">
                        ...
                        <select id="month" name="date" class="form-control form-control-sm rounded-0">
                    ...
                    <div class="col-lg-4 col-md-6 col-sm-12 col-xs-12">
                        <div class="form-group">
                            <button class="btn btn-primary rounded-50 btn-sm"><i class="fa fa-filter"></i> Filter</button>
                            <button class="btn btn-primary rounded-50 btn-sm" type="button" id="print"><i class="fa fa-print"></i> Print</button>
                        </div>
                    </div>
                </div>
            </form>
        ...
        <div class="container-fluid" id="printout">
			<table class="table table-hover table-striped table-bordered" id="report-list">
                ...
                <?php 
                        ...
                        $users_qry = $conn->query("SELECT id, concat(firstname, ' ', lastname) as `name` FROM `users` where id in (SELECT user_id FROM `sale_list` where MONTH(date_created) = '{$date}' {$where}) ");
                        $user_arr = array_column($users_qry->fetch_all(MYSQLI_ASSOC),'name', 'id');
						$qry = $conn->query("SELECT * FROM `sale_list` where MONTH(date_created) = '{$date}' {$where} order by unix_timestamp(date_updated) desc ");
						while($row = $qry->fetch_assoc()):
                            $total += $row['amount'];
					?>
						<tr>
                ...
                <tfoot>
                    <tr>
						...
						<th class="text-right"><?php echo format_num($total); ?></th>                       
                    </tr>
                </tfoot>
			</table>
		</div>
...
<noscript id="print-header">
    ...
    <div class="d-flex w-100">
        ...
        <div class="col-8 text-center" style="line-height:.9em">
            ...
            <h3 class="text-center m-0"><b>Monthly Sales Report</b></h3>
            <h5 class="text-center m-0"><b>as of</b></h5>
            <h3 class="text-center m-0"><b><?= date('F', mktime(0,0,0,$date)) ?></b></h3>
        </div>
    </div>
    <hr>
</noscript>
<script>
	$(document).ready(function(){
        ...
        $('#filter-form').submit(function(e){
            e.preventDefault()
            location.href = "./?page=reports/monthly&"+$(this).serialize()
        })
