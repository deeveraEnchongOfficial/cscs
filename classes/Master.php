...
Class Master extends DBConnection {
	...
	function save_sale(){
		...
		if($save){
			...
			if(isset($product_id)){
				...
				if(!empty($data)){
					//UPDATE product_list, sale_products SET product_list.`qty`=product_list.`qty`+sale_products.`qty` WHERE product_list.`id` = sale_products.`product_id` AND sale_products.`sale_id` = 24
					$this->conn->query("UPDATE product_list, sale_products SET product_list.`qty`=product_list.`qty`+sale_products.`qty` WHERE product_list.`id` = sale_products.`product_id` AND sale_products.`sale_id` = '{$sid}'");
					$this->conn->query("DELETE FROM `sale_products` where sale_id = '{$sid}'");
					$this->conn->query("UPDATE product_list SET qty=qty-{$qty} where id = {$pid}");
					//Console.log( "UPDATE product_list SET qty=qty-{$qty} where id = '{$pid}'");
	...
	function getTopSales(){
		...
		$inventory_result = $this->conn->query($sql);
...
switch ($action) {
	case 'get_top_sales':{
		echo "XXXXX";
		//echo $Master->getTopSales();
	break;
	}
	case 'get_inventory_chart':
		echo $Master->getInventoryChart();
	break;
	case 'delete_img':
